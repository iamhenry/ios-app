...
- Prefer Astro MCP for ASO keyword research:
  - `list_apps` — find tracked apps
  - `get_app_keywords` — current tracked keyword set
  - `search_rankings` with `includeHistory: true` — current and historical rank movement
  - `search_app_store` — live search results for a keyword/store
  - `get_keyword_suggestions` — AI keyword ideas for tracked or untracked apps
  - `extract_competitors_keywords` — competitor keyword ideas from tracked ranking data
- Use App Store Connect CLI only for official own-app context, not third-party keyword estimates:
  ```bash
  asc auth status --validate
  asc apps list --output json
  asc apps info view --app "APP_ID" --output json --pretty
  asc metadata keywords audit --app "APP_ID" --version "VERSION"
  ```
- If Astro MCP is unavailable, ask the user to enable Astro Settings → MCP Server and configure `http://127.0.0.1:8089/mcp`.
