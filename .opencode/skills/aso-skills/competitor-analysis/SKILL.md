...
- Use Astro MCP for competitor discovery and ASO evidence:
  - `search_app_store` — find competing apps for target keywords/store
  - `search_rankings` — compare keyword ranks; use `includeHistory: true` for trends
  - `extract_competitors_keywords` — identify competitor keyword opportunities
  - `get_app_ratings` — compare rating trend by store/country
  - `get_app_keywords` — compare tracked keyword coverage
- Use App Store Connect CLI for the user's official app data:
  ```bash
  asc auth status --validate
  asc apps list --output json
  asc apps info view --app "APP_ID" --output json --pretty
  asc reviews list --app "APP_ID" --output json --paginate
  ```
- Treat competitor download/revenue numbers as estimates unless they come from the user's own App Store Connect account.
