...
- Use Astro MCP for keyword evidence: `get_app_keywords`, `search_rankings`, `get_keyword_suggestions`, and `search_app_store`.
- Use App Store Connect CLI for official iOS metadata workflows:
  ```bash
  asc auth status --validate
  asc apps list --output json
  asc apps info view --app "APP_ID" --output json --pretty
  asc metadata init --dir "./metadata" --version "VERSION" --locale "en-US"
  asc metadata keywords audit --app "APP_ID" --version "VERSION" --blocked-terms-file "./blocked-terms.txt"
  asc metadata apply --app "APP_ID" --version "VERSION" --dir "./metadata" --dry-run
  ```
- Do not apply metadata changes without an explicit user confirmation; use `--dry-run` first.
