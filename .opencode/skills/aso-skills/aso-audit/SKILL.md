...
- Astro MCP for ASO market data:
  - `list_apps` — confirm tracked app
  - `get_app_keywords` — tracked keywords
  - `search_rankings` with `includeHistory: true` — rank health and trend
  - `get_app_ratings` with `includeHistory: true` — rating health and trend
  - `search_app_store` — category/keyword competitors
  - `extract_competitors_keywords` — keyword gap ideas
- App Store Connect CLI for official own-app data:
  ```bash
...
If a tool is unavailable, ask the user for the missing metadata or export. Do not invent rankings, ratings, downloads, or revenue.
